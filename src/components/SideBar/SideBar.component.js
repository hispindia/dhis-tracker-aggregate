      ...
      if (data.dataElementList) {
        ...
        data.dataElementList.dataElements.forEach((de) => {
          let attribute = de.attributeValues.filter(
            (attr) => attr.attribute.id == `${trackerToAggMapping}` //Tracker to Aggregate Mapping ../api/attributes.json
          );
        ...
        data.dataSetList.dataSets.forEach((dataSet) => {
          if (
            dataSet.attributeValues.find(
              (attrVal) =>
                attrVal.attribute.id == `${dataSetSharing}` && attrVal.value == "true" //Data Set Sharing ../api/attributes.json
            )
          ) {
